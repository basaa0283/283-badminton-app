...
## 6.5 通知・LINE チャネル方針 (2026-09-15 決定)

- **認証**: 全テナント共通の LINE Login チャネル (現 28ばど の env チャネル) を使う。
  テナント別チャネル (P3 実装) は「将来の上級オプション」としてコードは残すが、
  パイロット・一般テナントには案内しない。
  → 副作用として、全テナント同一プロバイダー認証になるため、同じ人が複数サークルに
    参加しても同一 User として扱える (大会実績のサークル横断共有が機能する)
  → 製品名決定時 (P4/ドメイン取得時) に LINE チャネルの表示名を製品名へ変更する
- **通知**: 他テナントは**メール通知のみ** (LINE Push は 28ばど 専用)。
  Messaging API のテナント別トークン解決は実装済みだが、未設定テナントはスキップされる
- **帰結**: パイロット受け入れで先方の LINE Developers 作業が不要になる
- **P5 優先課題**: リマインダー cron が現状デフォルトテナントのイベントしか対象に
  していないため、全テナントを対象にメールリマインダーを送るループ化が必要
...
## 7.5 ドメイン戦略 (2026-09-04 ブレスト済み)

方針: **製品ブランドのドメインを 1 つ取得し、パス方式で開始する** (P4 の一般公開準備時に実施)。

- サークル名由来の 28bad.net ではなく、**サービス名として中立なドメイン**を推奨
  (他サークルに売る製品であり、28ばど はテナントの 1 つという位置づけ)
- サービス名・ドメイン名の決定は P4 のタイミングで行う
- 段階:
  1. パス方式 `app.<製品ドメイン>/<slug>/...` — P2 実装がそのまま使える。
     Azure の無料マネージド証明書 1 枚で足りる。追加コストはドメイン代のみ (年 1,500〜2,000 円)
  2. 将来、見栄え重視でサブドメイン方式 `<slug>.<製品ドメイン>` へ移行可 —
     ワイルドカード TLS が必要 (Azure 無料証明書は非対応、Cloudflare 前段が現実解)。
     middleware をパス解決 → Host 解決に変える小規模改修で移行できる
- LINE コールバック URL は各テナント自前チャネル (P3) のためどちらの方式でも問題なし
