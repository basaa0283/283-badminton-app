...
- attending メンバーを paid にすると expenses.actualRevenue に反映される
- UI からイベント詳細で経費を編集・保存できる
- 経費レポートで合計が表示される

### `e2e/guest-role.spec.ts`

#### ゲストロール (閲覧専用)

- visibleToGuest=true のイベントだけ見え、出欠フォームが出ない
...
### `e2e/shuttle-prices.spec.ts`

#### シャトル単価マスタ (admin)

- 単価を追加→一覧に表示→削除でクリーンアップ
- 単価設定後のイベントで個数入力時にシャトル代が自動算出される
