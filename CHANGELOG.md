...
- お知らせの既読管理 + ヘッダー導線
  - `AnnouncementRead` テーブルでユーザー別の既読状態を保存
  - ホームバナーの絞り込みを「重要は常に表示、通常は未読のみ」に
  - `/announcements` を開くと表示中の未読を全件まとめて既読化
  - `/announcements` の各カードに「未読」バッジ
  - ヘッダーナビゲーションに「お知らせ」タブを追加
- イベント中止フラグ
  - 削除と区別して「中止」状態にできる。中止理由を任意で記録し、参加者に表示。
  - イベント詳細から「中止する」ボタン (admin/subadmin)、解除も可能
  - 一覧・詳細で「中止」バッジ + タイトル取り消し線で視認性向上
- 管理画面のメンバー一覧をテーブル形式に
  - ニックネーム / 権限 / 性別 / 年齢 / Lv / 最終操作 を一括表示
  - 各列クリックでソート切替、検索 / 権限 / 性別 でフィルタ
- ヘッダーのアバターメニューに「プロフィール」を追加
  - タブからプロフィールを除外し、メニュー内に集約
- ベータリリース向けの整備
